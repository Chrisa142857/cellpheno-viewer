...
const MainNiiView: React.FC<MainNiiViewProps> = React.memo(
  ({ volumes, onLocationChange, overlayUrl }) => {
    ...
    // Density blob(s) + the tile-boundary overlay (loaded straight from its URL so
    // changing it doesn't re-download the big density map).
    const canvasVolumes = useMemo(
      () =>
        // Only add the overlay once the density background is loaded (NiiVue errors
        // if the first/background volume is an overlay-only set).
    ...
    return (
      <div className="relative flex-1 h-full" onPointerDown={handleDown} onPointerUp={handlePick}>
        <NiivueCanvas volumes={canvasVolumes} options={FIXED_OPTIONS} onStart={configNiivue} />
        {pick && (
          <div
...
const ModulateScalar: React.FC<ModulateScalarProps> = ({
  ...
  // Stitching QC: per-tile nudge (orig px per tile-step) applied to the stitched
  // zoom cubes; and the tile-boundary overlay for the main view.
  const [stitchDx, setStitchDx] = useState(0);
  const [stitchDy, setStitchDy] = useState(0);
  const [tilesUrl, setTilesUrl] = useState<string>("");
  const [showTiles, setShowTiles] = useState(true);
  const lastVoxRef = useRef<number[] | null>(null);
  ...
  const fetchZoomData = useCallback(
    async (coordinates: number[]) => {
      lastVoxRef.current = coordinates;
      const coordinatesKey = `${coordinates.join("_")}_${stitchDx}_${stitchDy}`;
      ...
      try {
        ...
        // Tile-boundary overlay from the 1x (detail) scale, so it marks only the
    ...
    [brainId, setZoomViews, setZoomCtl, stitchDx, stitchDy]
  );

  // Re-fetch the zoom cubes when the stitch nudge changes (same clicked spot).
  useEffect(() => {
    if (lastVoxRef.current && lastVoxRef.current.length >= 3) {
      fetchZoomData(lastVoxRef.current);
    }
        ...
        <ViewHeader
          ...
          layers={[
            ...Object.entries(mainVolumes).map(([key, vol]) => ({
              ...
              onToggle: (vis: boolean) => handleMainVolumeChange(key, { opacity: vis ? 1 : 0 }),
            })),
            ...(tilesUrl
              ? [{ label: "Tiles", visible: showTiles, onToggle: (v: boolean) => setShowTiles(v) }]
              : []),
          ]}
          ...
          <div className="flex shrink-0 items-center gap-3 rounded-lg bg-slate-900/50 px-3 py-1 text-[11px] text-slate-300 ring-1 ring-cyan-500/10">
            ...
            <label className="flex items-center gap-1">
              ...
              <input
                ...
                step={2}
                value={stitchDx}
                onChange={(e) => setStitchDx(Number(e.target.value))}
                className="w-24 accent-cyan-400"
              />
              <span className="w-10 tabular-nums text-slate-400">{stitchDx}px</span>
            </label>
            <label className="flex items-center gap-1">
              dy
              <input
                ...
                step={2}
                value={stitchDy}
                onChange={(e) => setStitchDy(Number(e.target.value))}
                className="w-24 accent-cyan-400"
              />
              <span className="w-10 tabular-nums text-slate-400">{stitchDy}px</span>
            </label>
            <button
              type="button"
              onClick={() => {
              ...
              className="rounded px-1.5 py-0.5 ring-1 ring-slate-700 hover:bg-cyan-500/10 hover:text-cyan-200"
            >
              reset
            </button>
